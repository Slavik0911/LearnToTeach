...
export default function Profile() {
    ...
    async function renameFolder(folder, newName) {
        try {
            ...
            setFolders((prev) =>
                prev.map((item) =>
                    item.id === folder.id ? { ...item, title: newValue } : item,
                ),
            );
    ...
    async function deleteFolder(folder) {
        try {
            ...
            const deletePromises = lessons.map((l) =>
                deleteDoc(
                    doc(
                        db,
                        "users",
        ...
        <>
            <div className="mx-auto">
                        ...
                        <div className="bg-gray-100 px-10 pb-10 pt-0 rounded-2xl">
                            ...
                            <StatGrid cols={1} smCols={2} lgCols={3}>
                                ...
                                <StatCard
                                    ...
                                    value={recentlyWatchedCount}
                                    note=""
                                    route="/recently-watched"
                                />
                                <StatCard
                                    title="Purchased"
                                    value={profile.purchasedCount}
                                    note=""
                                    route="/purchased"
                                />
                            </StatGrid>
                ...
                <RenameModal
                    ...
                    onConfirm={(newName) =>
                        renameFolder(editingFolder, newName)
                    }
                />
                <ConfirmModal
                    open={!!deletingFolder}
